...
export const RevenueTrendChart: React.FC<RevenueTrendChartProps> = ({ data, isLoading }) => {
    ...
    return (
      <Card>
        <div className="h-80 flex flex-col justify-center items-center text-center">
          ...
          <p className="text-sm text-text-muted">Not enough data to generate a trend chart.</p>
        </div>
      </Card>
    );
  }

  // Pad single-point datasets with bookend entries so Recharts renders a line
  const chartData =
    data.length === 1
      ? [
  ...
  return (
    <Card>
      <h3 className="text-base font-semibold text-text-primary mb-lg">Revenue & Profit Trend</h3>
